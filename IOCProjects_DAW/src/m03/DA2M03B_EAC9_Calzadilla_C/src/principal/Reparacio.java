...
public class Reparacio implements Component{
    ...
    public void addMecanic(Mecanic mecanic) throws GestorTallerMecanicException{ // SE DEBE DE HACER EL LLAMADO A LA EXCEPCION
     ...
     for(int i = 0; i < maps.get("mecanics").size(); i++) {   
        if(((Mecanic) maps.get("mecanics").get(i)).getNif().equals(mecanic.getNif())) {//RECORRE EL MAP CON LA CLAVE Y LA LISTA INTERNA DEL ARRAYLIS UBICANDO SI EL NIF ESTA O NO REPETIDO
    ...
    for(int i = 0; i < maps.get("recanvis").size(); i++) {
        if(((Recanvi) maps.get("recanvis").get(i)).getCodi().equals(recanvi.getCodi())) {// VERIFICA SI EL CODIGO YA ESTA SIGNADO
    ...
    maps.get("recanvis").add(recanvi); // LA ADICION DE UN NUEVO VALOR EN EL ARRYLIST SE HA DE REALIZAR FUERA DEL FOR PARA QUE INCREMENTE EN VALOR LA I
    ...
    public void showComponent(){
        ...
        if(!maps.get("recanvis").isEmpty()) {
            ...
            System.out.println("\n El recambio no se encuentra asignado");
        }
